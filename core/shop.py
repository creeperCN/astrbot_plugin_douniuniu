...
from .do_other import DoOther
from .do_self import DoSelf
from .utils import probabilistic_decision, get_add_text, check_cooldown, \
...
class Shop:
    def __init__(self):
        ...
        self.items = {
            ...
            2: {"name": "迷幻菌子", "price": 150, "description": "使用后全群陷入迷幻，立即消耗本群5个人的打胶/自摸机会为自己打胶/自摸"},
            3: {"name": "春天的药", "price": 150, "description": "使用后变得欲求不满，随机挑选5人为自己锁牛牛/吸猫猫"},
            4: {"name": "黑店壮丁手术体验卡", "price": 100, "description": "获得一次乡下黑店的牛牛手术机会，30%概率长度+20%（单次上限100cm），70%概率长度-10%"},
            5: {"name": "诊所壮丁手术体验卡", "price": 200, "description": "获得一次小镇诊所的牛牛手术机会，50%概率长度+20%（单次上限100cm），50%概率长度-10%"},
            6: {"name": "医院壮丁手术体验卡", "price": 400, "description": "获得一次正规医院的牛牛手术机会，70%概率长度+20%（单次上限100cm），30%概率长度-10%"},
            7: {"name": "六味地黄丸", "price": 100, "description": "下次由你主动发起的比划必胜"},
            8: {"name": "负重沙袋", "price": 100, "description": "一个可以使牛牛下次锻炼效率翻倍的道具"},
            9: {"name": "会跳的蛋", "price": 100, "description": "一个可以使猫猫下次锻炼效率翻倍的道具"},
            10: {"name": "性转针筒", "price": 100, "description": "一针下去24h内牛牛将暂时缩腹为猫，期间打工金币翻倍，再次使用将变回牛牛"},
            11: {"name": "牛牛转换器", "price": 500, "description": "可以交换评分和自己相差不超过20%的目标用户的牛牛属性"},
            12: {"name": "猫猫转换器", "price": 500, "description": "可以交换评分和自己相差不超过20%的目标用户的猫猫属性"},
            13: {"name": "春风精灵", "price": 50, "description": "1小时内每次冷却完毕自动打胶/自摸，输入“/春风精灵提醒 开/关”设置是否提醒"},
    ...
    def get_items(self, user_id) -> str:
        ...
        is_discount = user_data['items'].get('20off', False)
        ...
        for key, value in self.items.items():
            if is_discount:
                discount_price = int(value['price'] * 0.8)
                text += f"🛒 {key}. {value['name']} 💰️ ~{value['price']}~ -> {discount_price}\n"
            else:
    ...
    def use_big_d(self, group_id, user_id, success_prob: float) -> str:
        ...
        if is_add:
            # 数值平衡：+20% 且单次上限100cm，防止翻倍式指数爆炸
            add_length = max(1, int(user_data['length'] * 0.2))
            add_length = min(add_length, 100)
            true_add = self.data_manager.add_length(group_id, user_id, add_length)
            text += f"🏥 手术成功\n"
            new_user_data = self.data_manager.get_user_data(user_id)
            text += get_add_text(true_add, add_length, new_user_data)
        else:
            del_length = max(1, int(user_data['length'] * 0.1))
            self.data_manager.del_length(user_id, del_length)
            text += f"🏥 手术失败\n"
            text += f"📏 {user_data['niuniu_name']}长度减少{del_length}cm，当前长度：{format_length(self.data_manager.get_user_data(user_id)['length'])}"
        return text
    ...
    def use_aphrodisiac(self, group_id, user_id, do_other_cd) -> str:
        ...
        for can_do_id in can_do_other_list:
            ...
            if not user_data['items']['transfer']:
                # 春天的药为付费道具效果，绕过被锁方免疫
                text += self.do_other.do_other_niu(group_id, can_do_id, user_id, do_other_cd, bypass_victim_cd=True)
            else:
                text += self.do_other.do_other_mao(group_id, can_do_id, user_id, do_other_cd, bypass_victim_cd=True)
            text += '\n'
        return text
